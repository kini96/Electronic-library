...
module.exports = function(config) {
	...
	User.find({}).exec(function(err, collection) {
		...
		if (collection.length === 0) {
			...
			User.create({username: 'iana.st', firstName: 'Iana', lastName: 'Stoyanova', salt: salt, hashPass: hashedPwd});
			salt = generateSalt();
			hashedPwd = generateHashedPassword(salt, 'Sadika');
			User.create({username: 'kini96', firstName: 'Sadika', lastName: 'Madjarova', salt: salt, hashPass: hashedPwd});
